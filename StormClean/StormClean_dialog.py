...
class StormCleanDialog(QtGui.QDialog, FORM_CLASS):
    ...
    def showEvent(self, event):
        ...
        uf.insertTempFeatures(onestartpointlayer, [startpoint], [['Startlocation']])
    ...
    def topage1(self):
        ...
        for att in wijkname:
            current = att.attribute('WK_NAAM')
            self.label_location.setText('<p><strong>You are currently in ' + current +'</strong></p>')
            global current
        ...
        self.listWidget_listofneighbourhood.sortItems()
        ...
        if listitem == None:
            ...
            if ret == 0:
                ...
                for att in attr:
                    ...
                    reported = att.attribute('TimesMenti')
                    self.textBrowser.setHtml(
                        "<p><strong>Additional information</strong></p>" + "First reported: " + str(
                            time) + " | Reported " + str(reported) + " times")
                    if attribute != NULL:
    ...
    def inwijk(self):
        ...
        processing.runandload('qgis:joinattributesbylocation', 'OneStartPoint','Wijk_Rott', u'within', 0, 0, '', 0,
                              os.path.join(os.path.dirname(__file__), 'SampleData', 'Joined layer.shp'))

    def newlocation(self):
        pointlayer = uf.getLegendLayerByName(self.iface, "Joined layer")
        backuppl = uf.getLegendLayerByName(self.iface, "OneStartPoint")

        backuppl.startEditing()
        ids = uf.getAllFeatureIds(backuppl)
        for id in ids:
            backuppl.deleteFeature(id)
        backuppl.commitChanges()
        ...
        uf.insertTempFeatures(backuppl, [location], [["new start location"]])
